...
! Fill ghost cells with appropriate boundary values.
    ...
    do i = 1, meqn
        call fill_component_ghost_cells(bc_options(:, i),  &
                                        lower_values(:, i),  &
                                        upper_values(:, i),  &
                                        q(:, i))
    end do
...
! Fill ghost cells with homogeneous boundary conditions.
    ...
    call fill_component_ghost_cells(bc_options, zeros, zeros, q_component)

end subroutine fill_component_ghost_cells_homogeneous


subroutine fill_component_ghost_cells(bc_options, lower_values, upper_values, q)
